...
def keyword_highlighter(user_sentence, top3_json, model_path):
  ...
  top3_dic = top3_json
  for i in range(0,len(top3_dic)):
    tr4w.analyze(top3_dic[i]['review'], candidate_pos = ['NOUN', 'PROPN', 'ADJ'], window_size=5, lower=False)
  ...
  color_list = []
  for i in range(0, len(top3_dic)):
    top3_dict = {i:None for i in top3_dic[i]['keywords']}
    index = 0
  ...
  result = {'text': [user_sentence],
            'highlight': [user_dict]}
  result = pd.DataFrame(result)

  for i in range(0, len(top3_dic)):
    color_dic = {}
    perfume = color_list[i]
    for tw in perfume.keys():
      if perfume[tw] is not None:
        color_dic[tw] = perfume[tw][1]
    result = result.append({'text':top3_dic[i]['review'], 'highlight':color_dic}, ignore_index=True)
...
if __name__=="__main__":
    ...
    label = 2
    top3_json = word2vec_similarity.word2vec_similarity(user_sentence, label)
    model_path = './model/w2v_10window'
    pprint.pprint(keyword_highlighter(user_sentence, top3_json, model_path))